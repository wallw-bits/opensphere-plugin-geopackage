...
describe('plugin.geopackage.GeoPackageDescriptor', function() {
  it('should report the correct type', function() {
    var d = new plugin.geopackage.GeoPackageDescriptor();
  ...
  it('should produce the correct layer options', function() {
    ...
    expect(opts.type).toBe(plugin.geopackage.ID);
  ...
  it('should create a descriptor from a file parser config', function() {
    ...
    file.setUrl('http://localhost/doesnotexist.geopackage');

    // default config
    var config = new os.parse.FileParserConfig(file);
    var d = plugin.geopackage.GeoPackageDescriptor.createFromConfig(config);

    expect(d.getId()).toBeTruthy();
    expect(d.getProvider()).toBe(plugin.geopackage.GeoPackageProvider.getInstance().getLabel());
    ...
    d = plugin.geopackage.GeoPackageDescriptor.createFromConfig(config);
    expect(d.getId()).toBeTruthy();
    expect(d.getProvider()).toBe(plugin.geopackage.GeoPackageProvider.getInstance().getLabel());
    expect(d.getUrl()).toBe(file.getUrl());
    expect(d.getColor()).toBeTruthy();
